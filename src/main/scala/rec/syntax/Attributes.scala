...
import rec.Attr
import rec.Attr.{Attribute, Property}
...
object Attributes {
  class AttributeMaker(key: String) {
    def apply(value: String): Attr =
      Attribute(key, value)
    val empty: Attr = Attribute(key, "")
  }
  private def attr(key: String): AttributeMaker = new AttributeMaker(key)

  class PropertyMaker(key: String) {
    def apply(value: String | Boolean | Int): Attr =
      Property(key, value)
    val empty: Attr = Property(key, "")
  }
