...
class EventBindMaker(val key: String) {
  def apply(value: dom.Event => Html[_]): EventBind = EventBind(key, value)
}
trait Events {
  private def event(key: String): EventBindMaker = new EventBindMaker(key)
