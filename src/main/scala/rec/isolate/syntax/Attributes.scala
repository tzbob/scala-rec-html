...
object Attributes {
  class AttributeMaker(key: String) {
    def apply(value: String): Attribute[Nothing, HNil] =
      Attribute.AttributeImpl(key, value)
    val empty: Attribute[Nothing, HNil] = Attribute.AttributeImpl(key, "")
  }
  def attr(key: String): AttributeMaker = new AttributeMaker(key)

  class PropertyMaker(key: String) {
    def apply(value: String | Boolean | Int): Attribute[Nothing, HNil] =
      Attribute.Property(key, value)
    val empty: Attribute[Nothing, HNil] = Attribute.Property(key, "")
  }
  ...
  /**
    ...
    */
  trait InputAttrs extends Global {
