...
import shapeless.ops.record.{DeepMerger, Keys}
...
object RecordConcat {
  ...
  private[rec] def dummy[R1 <: HList, R2 <: HList, R <: HList] = new RecordConcat[R1, R2, R] {
    override def apply(r1: R1, r2: R2): R = ???
  }
...
//      align: Align[SubResult, Result]): RecordConcat[R1, R2, Result] =
      merger: DeepMerger.Aux[R1, R2, Result]): RecordConcat[R1, R2, Result] =
    new RecordConcat[R1, R2, Result] {
//      override def apply(r1: R1, r2: R2): Result = align(prepended(r1, r2))
      override def apply(r1: R1, r2: R2): Result = merger.apply(r1, r2)
    }
